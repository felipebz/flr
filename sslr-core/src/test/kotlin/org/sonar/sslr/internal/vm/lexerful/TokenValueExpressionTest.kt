...
class TokenValueExpressionTest {
    ...
    fun should_compile() {
        ...
        Assertions.assertThat(expression.toString()).isEqualTo("TokenValue foo")
    }
    ...
    fun should_backtrack2() {
        val token = Mockito.mock(Token::class.java)
        Mockito.`when`(token.value).thenReturn("bar")
        Mockito.`when`(machine.length).thenReturn(1)
        Mockito.`when`(machine.tokenAt(0)).thenReturn(token)
        expression.execute(machine)
        val inOrder = Mockito.inOrder(machine)
    ...
    fun should_backtrack3() {
        val token = Mockito.mock(Token::class.java)
        Mockito.`when`(token.value).thenReturn("h31" /* same hash code as for "foo" */)
        Mockito.`when`(machine.length).thenReturn(1)
        Mockito.`when`(machine.tokenAt(0)).thenReturn(token)
        expression.execute(machine)
        val inOrder = Mockito.inOrder(machine)
